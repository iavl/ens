...
contract HashRegistrar is Registrar {
    ...
    uint256 constant minPrice = 0.01 ether;
    uint256 public registryStarted;
    ...
    modifier onlyOwner(bytes32 _hash) {
        require(
            state(_hash) == Mode.Owned &&
                msg.sender == _entries[_hash].deed.owner()
        );
        _;
    }
    ...
    modifier registryOpen() {
        require(
            now >= registryStarted &&
                now <= registryStarted + (365 * 4) * 1 days &&
    ...
    function unsealBid(
        ...
        uint256 value = min(_value, bid.value());
        ...
        if (auctionState == Mode.Owned) {
            ...
            value < minPrice ||
            bid.creationDate() > h.registrationDate - revealPeriod
        ) {
     ...
     */
    function transfer(bytes32 _hash, address payable newOwner)
        external
        onlyOwner(_hash)
    {
    ...
    function releaseDeed(bytes32 _hash) external onlyOwner(_hash) {
        ...
        require(
            now >= h.registrationDate + 365 days ||
                ens.owner(rootNode) != address(this)
        );
    ...
    function transferRegistrars(bytes32 _hash) external onlyOwner(_hash) {
        ...
        Registrar(registrar).acceptRegistrarTransfer(
            _hash,
            h.deed,
            h.registrationDate
        );
    ...
    function acceptRegistrarTransfer(
        ...
        hash;
        deed;
        registrationDate; // Don't warn about unused variables
    }
     ...
     */
    function isAllowed(bytes32 _hash, uint256 _timestamp)
        public
        view
     ...
     * @param _hash The hash to start an auction on
     */
    function getAllowedTime(bytes32 _hash) public view returns (uint256) {
        return
