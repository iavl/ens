...
contract RegistrarMigration {
    ...
    bytes private constant UNUSED_SUBDOMAIN = hex"ffffffffffffffff";
    ...
    constructor(
        OldBaseRegistrarImplementation _old,
        BaseRegistrarImplementation _new,
        AbstractSubdomainRegistrar _oldSubdomainRegistrar,
        AbstractSubdomainRegistrar _newSubdomainRegistrar
    ) public {
        oldRegistrar = _old;
    ...
    function doMigration(
        ...
        if (
            address(registrant) != address(oldSubdomainRegistrar) &&
            hasCode(controller)
        ) {
        ...
        if (
            ...
            ) = oldSubdomainRegistrar.query(
                    bytes32(tokenId),
                    string(UNUSED_SUBDOMAIN)
                );
            ...
            newSubdomainRegistrar.configureDomainFor(
                label,
                price,
                referralFeePPM,
    ...
    function migrateLegacy(bytes32 label) public {
        (Registrar.Mode mode, address deed, , , ) = legacyRegistrar.entries(
            label
        );
