...
contract DeedImplementation is Deed {
    ...
    function setBalance(uint256 newValue, bool throwOnFailure)
        external
        onlyRegistrar
        onlyActive
    {
        ...
        require(
            _owner.send(address(this).balance - newValue) || !throwOnFailure
        );
    }
     ...
     */
    function closeDeed(uint256 refundRatio) external onlyRegistrar onlyActive {
        active = false;
        require(
            burn.send(((1000 - refundRatio) * address(this).balance) / 1000)
        );
        emit DeedClosed();
        _destroyDeed();
    }
